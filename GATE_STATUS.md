...
This ledger documents the verification status across all phases and quality gates of the Amend project following the macOS 26+ native modernization and product rebrand. Amend is built natively against official Apple macOS 26+ APIs (`SwiftUI.Glass`, `SwiftUI.GlassEffectContainer`, `View.glassEffect`, and `FoundationModels`). Zero fake compatibility shims or SDK fallbacks exist in Amend production code.

- **Product**: Amend
- **Repository**: `Fadyio/amend`
- **Minimum Deployment Target**: macOS 26.0
- **Package**: Amend
- **Core Module**: `AmendCore`
- **App Module**: `AmendApp`
- **Executable Target / Binary**: `Amend`
- **Application Bundle**: `dist/Amend.app`
- **Default Installed App**: `~/Applications/Amend.app`
- **Project Bundle**: `.amend`
- **Bundle ID**: `com.fady.amend`
- **Project UTI**: `com.fady.amend.project`
- **Swift Tools Version**: 6.0 (`swift-tools-version: 6.0`)
- **Swift Language Mode**:
  - `AmendCore`: Swift 6 (`.swiftLanguageMode(.v6)`)
  - `AmendApp`: Swift 6 (`.swiftLanguageMode(.v6)`)
  - `Amend` executable: Swift 6 (`.swiftLanguageMode(.v6)`)
  - `AmendCoreTests`: Swift 5 (`.swiftLanguageMode(.v5)`) for async test harness & mock URL protocol concurrency compatibility
...
- **Verification Categorization & Test Accounting**:
  1. **DETERMINISTIC CI** (`swift test --no-parallel`):
     - **276 passed**, 0 failed, 4 opt-in acceptance tests skipped by default via Swift Testing `.enabled(if:)` traits.
     - Runs fully automated in CI and local machines without requiring local neural weights or real media.
  2. **LOCAL NEURAL ACCEPTANCE** (`AMEND_RUN_LOCAL_AI_TESTS=1`):
  ...
  3. **APPLE FOUNDATION MODELS ACCEPTANCE** (`AMEND_RUN_APPLE_MODEL_TESTS=1`):
     - 1 opt-in acceptance test:
       - `AppleFoundationGrammarTests.test_live_apple_foundation_model_inference`: Executes live on-device Apple Intelligence grammar rewrite via native `FoundationModels` (`SystemLanguageModel.default`, `LanguageModelSession`, schema-constrained generation) without any compatibility shims (passed live in 26.70s and 30.41s). Fails explicitly if `SystemLanguageModel` is unavailable.
  4. **REAL-MEDIA ACCEPTANCE** (`AMEND_RUN_REAL_MEDIA_TESTS=1` + `AMEND_REAL_MEDIA="/path/to/file.mov"`):
     - 1 opt-in acceptance test:
       - `RealWorldAcceptanceTests.test_real_media_acceptance_journey`: Validates live AVFoundation video frame decoding, timeline scrubbing, and deliberate transcription against user recording. Fails explicitly if `AMEND_REAL_MEDIA` is missing or unreadable.
...
| **Project Bundle Persistence** | **VERIFIED** | `.amend` package saves/loads media reference, tracks, cues, candidate WAVs, Reference Voice, and provider IDs; verified in `AmendProjectFormatTests` and `StorageAPFSTests`. |
...
| **PocketTTS Engine Seam & Cache** | **VERIFIED** | Amend-owned `PocketTTSEngine` abstraction with `SpyPocketTTSEngine` test seam; `Packages/FluidAudio` pristine with zero diff. |
| **PocketTTS Voice Cloning Acceptance** | **REAL 8 GB M1 WORKFLOW EXECUTED SUCCESSFULLY** | Executed live on Apple Silicon M1 (`AMEND_RUN_LOCAL_AI_TESTS=1 swift test --filter PocketTTSAcceptanceTests` passed in 1179.8s). |
...
| **Apple Foundation Models Grammar** | **REAL ON-DEVICE WORKFLOW EXECUTED SUCCESSFULLY** | Implemented in `AppleFoundationGrammarProvider` via native `FoundationModels` framework; verified on-device with structured generation schema and diff computation (`AMEND_RUN_APPLE_MODEL_TESTS=1` passed in 26.70s and 30.41s with zero shims). |
...
- **Test Suites**:
  ...
  - `AmendProjectFormatTests` (4/4 passed)
- **Key Deliverables**:
  - APFS CoW cloning (`APFSCloner`) with security-scoped bookmark fallback (`BookmarkManager`).
  - Strict Keychain credential storage (`KeychainVault`, `kSecClassGenericPassword`, service `com.fady.amend`).
  - Zero-credential bundle leak scanner (`CredentialLeakScanner`) detecting provider patterns, `.env` files, and absolute user paths.
  - Robust project bundle serialization (`ProjectBundleSerializer`, `ProjectBundle`, `ProjectMetadata`) with `.amend` package format.
...
- **Key Deliverables**:
  - Modularized `AmendApp` library target allowing SPM test target to test top-level `AppViewModel` and UI coordinators.
...
| :--- | :--- | :--- | :--- | :--- |
| **Gate A** | `swift build` succeeds for AmendCore, AmendApp, and Amend executable | `Package.swift`, `Sources/AmendCore/`, `Sources/AmendApp/`, `Sources/Amend/` | Full SPM target compilation (`swift build`) | **IMPLEMENTED + VERIFIED** |
| **Gate B** | All deterministic unit/integration tests pass | `Tests/AmendCoreTests/` (29 test suites) | `swift test --no-parallel` (276/276 passed, 4 acceptance tests skipped) | **IMPLEMENTED + VERIFIED** |
| **Gate C** | Assembled app-level E2E journey passes | `Sources/AmendApp/ViewModels/AppViewModel.swift` | `AssembledAppE2ETests.test_complete_sixteen_step_assembled_user_journey` | **IMPLEMENTED + VERIFIED** |
| **Gate D** | Real selected-track routing is verified | `Sources/AmendCore/Composition/AudioTrackExtractor.swift`, `Sources/AmendCore/Transcription/CueGenerator.swift` | `AudioRoutingTests` (zero-crossing routing verification on Track B) | **IMPLEMENTED + VERIFIED** |
| **Gate E** | Real Silero VAD is verified | `Sources/AmendCore/Transcription/SilenceDetector.swift` | `TranscriptionVADTests` (compiled Core ML model on Neural Engine/CPU rejecting tones) | **IMPLEMENTED + VERIFIED** |
| **Gate F** | Real PocketTTS synthesis and local cloning work on target Mac | `Sources/AmendCore/Synthesis/VoiceSynthesizer.swift`, `PocketTTSProvider` | `PocketTTSAcceptanceTests` & `AssembledAppE2ETests` (opt-in; downloaded models, cloned Kathleen voice, synthesized audio on M1 Neural Engine) | **IMPLEMENTED + VERIFIED (REAL 8 GB M1 WORKFLOW EXECUTED SUCCESSFULLY)** |
| **Gate G** | Preview audibly/analytically contains replacement narration | `Sources/AmendCore/Composition/PreviewComposition.swift` | `PreviewCompositionTests` (zero-crossing frequency assertions) | **IMPLEMENTED + VERIFIED** |
| **Gate H** | Export analytically contains replacement narration | `Sources/AmendCore/Export/PassthroughExportPipeline.swift` | `PassthroughExportTests` (narration rebuilt, passthrough preserved) | **IMPLEMENTED + VERIFIED** |
| **Gate I** | Video compressed-sample identity is genuinely verified where passthrough is expected | `Sources/AmendCore/Export/PassthroughExportPipeline.swift` | `PassthroughExportTests.test_passthrough_export_preserves_exact_compressed_sample_hashes` | **IMPLEMENTED + VERIFIED** |
| **Gate J** | Save/load roundtrip preserves source resolution, Cue audio, Reference Voice & provider settings | `Sources/AmendCore/Storage/ProjectBundleSerializer.swift`, `AppViewModel.saveProject` | `StorageAPFSTests`, `AmendProjectFormatTests`, `ProviderAndOverflowBlockerTests.test_reference_voice_and_persistence_roundtrip` | **IMPLEMENTED + VERIFIED** |
| **Gate K** | Credential scanner proves no API credential is present in repository or `.amend` output | `Sources/AmendCore/Storage/CredentialLeakScanner.swift` | `SecuritySuiteTests`, `AssembledAppE2ETests` (step 16) | **IMPLEMENTED + VERIFIED** |
| **Gate L** | Live Gemini Grammar & TTS tests pass with real user-supplied Keychain credential | `Sources/AmendCore/Synthesis/VoiceSynthesizer.swift` (`GeminiTTSProvider`), `GrammarRewriter.swift` | Registered mock transport & negative auth rejection verified; live call pending user key | **IMPLEMENTED — LIVE CREDENTIAL VERIFICATION REQUIRED** |
| **Gate M** | Live ElevenLabs test passes when configured | `Sources/AmendCore/Synthesis/VoiceSynthesizer.swift` (`ElevenLabsProvider`) | Registered mock transport, clone & synth contract verified; live call pending user key | **IMPLEMENTED — LIVE CREDENTIAL VERIFICATION REQUIRED** |
| **Gate N** | Live Resemble test passes only when configured and account capabilities permit it | `Sources/AmendCore/Synthesis/VoiceSynthesizer.swift` (`ResembleProvider`) | Registered mock transport & /synthesize contract verified; live call pending user key | **IMPLEMENTED — LIVE CREDENTIAL VERIFICATION REQUIRED** |
| **Gate O** | Apple Foundation Models on-device structured grammar rewrite succeeds | `Sources/AmendCore/Synthesis/AppleFoundationGrammarProvider.swift` | `AppleFoundationGrammarTests` (deterministic mocks + opt-in real on-device model execution) | **IMPLEMENTED + VERIFIED (REAL ON-DEVICE WORKFLOW EXECUTED SUCCESSFULLY)** |
| **Gate P** | macOS 26 Liquid Glass UI styling and container controls | `Sources/AmendApp/Views/LiquidGlassTheme.swift`, `AmendToolbar.swift`, `CueInspectorView.swift`, etc. | Verified via packaged app launch and UI snapshot rendering | **IMPLEMENTED + VERIFIED** |
| **Gate Q** | Amend Project bundle format and system file association | `Packaging/Info.plist`, `ProjectBundle.swift` | `AmendProjectFormatTests` (.amend format, com.fady.amend.project UTI, URL routing) | **IMPLEMENTED + VERIFIED** |
