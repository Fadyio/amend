...
public struct GlassPanelModifier: ViewModifier {
    ...
    public init(cornerRadius: CGFloat = AmendTheme.cornerRadiusMedium) {
...
public struct ElevatedGlassPanelModifier: ViewModifier {
    ...
    public init(cornerRadius: CGFloat = AmendTheme.cornerRadiusLarge) {
        self.cornerRadius = cornerRadius
    }
...
public struct SelectedGlassSurfaceModifier: ViewModifier {
    ...
    public init(
        isSelected: Bool,
        isActive: Bool = false,
        cornerRadius: CGFloat = AmendTheme.cornerRadiusMedium
    ) {
    ...
    public func body(content: Content) -> some View {
        ...
        if isSelected {
            content
                .glassEffect(.regular.tint(AmendTheme.accent.opacity(0.35)), in: shape)
                .overlay(
                    shape.stroke(AmendTheme.accent.opacity(0.85), lineWidth: 1.5)
                )
        } else if isActive {
            content
                .glassEffect(.regular.tint(AmendTheme.accent.opacity(0.12)), in: shape)
                .overlay(
...
public extension View {
    func glassPanel(cornerRadius: CGFloat = AmendTheme.cornerRadiusMedium) -> some View {
        modifier(GlassPanelModifier(cornerRadius: cornerRadius))
    }

    func elevatedGlassPanel(cornerRadius: CGFloat = AmendTheme.cornerRadiusLarge) -> some View {
...
public struct StatusBadge: View {
    ...
    private var indicatorColor: Color {
        ...
        case .edited:
            return AmendTheme.statusWarning
        case .synthesized:
            return AmendTheme.statusSuccess
        case .overflowGated:
            return AmendTheme.statusError
        case .forceFitted:
            return Color.purple
        }
    }
...
public struct GlassButtonStyle: ButtonStyle {
    ...
    public func makeBody(configuration: Configuration) -> some View {
        let shape = RoundedRectangle(cornerRadius: AmendTheme.cornerRadiusSmall, style: .continuous)
        configuration.label
            ...
            .glassEffect(
                isProminent
                    ? .regular.tint(AmendTheme.accent).interactive()
                    : .regular.interactive(),
                in: shape
            )
