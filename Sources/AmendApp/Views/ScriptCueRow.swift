...
public struct ScriptCueRow: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                ...
                if isActive {
                    ...
                    Circle()
                        .fill(AmendTheme.accent)
                        .frame(width: 6, height: 6)
                }

                Text("#\(index + 1)")
                    .font(.system(size: 11, weight: .bold, design: .monospaced))
                    .foregroundStyle(isSelected ? AmendTheme.accent : .secondary)
        ...
        .padding(.vertical, 10)
        .selectedGlassSurface(isSelected: isSelected, isActive: isActive, cornerRadius: AmendTheme.cornerRadiusMedium)
        ...
        if isActive {
            if let targetWord = findActiveWord(), !targetWord.isEmpty {
                if let range = attr.range(of: targetWord) {
                    attr[range].foregroundColor = AmendTheme.accent
