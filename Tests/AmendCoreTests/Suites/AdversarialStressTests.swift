...
final class AdversarialStressTests {
    ...
    init() throws {
        ...
        self.tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("AmendAdversarialTests_\(uniqueID)", isDirectory: true)
    ...
    func test_apfs_cloning_readonly_source() throws {
        ...
        let bundleURL = tempDirectory.appendingPathComponent("ReadOnlyBundle.amend")
    ...
    func test_apfs_cloning_nested_bundle_path() throws {
        ...
        let nestedBundleURL = tempDirectory
            ...
            .appendingPathComponent("NestedProject.amend")
    ...
    func test_apfs_cow_mutation_independence() throws {
        ...
        let bundleURL = tempDirectory.appendingPathComponent("CoWProject.amend")
        let bundle = try ProjectBundleSerializer.createBundle(
            at: bundleURL,
            sourceMediaURL: sourceURL,
            name: "CoWProject",
    ...
    func test_apfs_cloning_special_characters_filename() throws {
        ...
        let bundleURL = tempDirectory.appendingPathComponent("Special Project [Alpha] ✨.amend")
        let bundle = try ProjectBundleSerializer.createBundle(
            at: bundleURL,
            sourceMediaURL: sourceURL,
    ...
    func test_atomic_serialization_concurrent_reads() async throws {
        let bundleURL = tempDirectory.appendingPathComponent("AtomicProject.amend")
    ...
    func test_keychain_vault_whitespace_handling() {
        let vault = KeychainVault(serviceIdentifier: "com.fady.amend.adversarial.test.\(UUID().uuidString)")
        defer {
    ...
    func test_project_bundle_load_corrupt_json() throws {
        let fakeBundleURL = tempDirectory.appendingPathComponent("Corrupt.amend")
    ...
    func test_keychain_vault_special_characters_and_emojis() throws {
        let vault = KeychainVault(serviceIdentifier: "com.fady.amend.adversarial.special.\(UUID().uuidString)")
        defer {
            for service in ServiceKey.allCases {
                try? vault.delete(keyFor: service)
            }
    ...
    func test_keychain_vault_multiple_redundant_deletions() throws {
        let vault = KeychainVault(serviceIdentifier: "com.fady.amend.adversarial.redundant.\(UUID().uuidString)")
    ...
    func test_credential_leak_scanner_bundle_user_path() throws {
        let bundleURL = tempDirectory.appendingPathComponent("UserPathLog.amend")
        let metadata = ProjectMetadata(
    ...
    func test_credential_leak_scanner_env_file_detection() throws {
        let bundleURL = tempDirectory.appendingPathComponent("EnvBundle.amend")
        let metadata = ProjectMetadata(
