...
struct AssembledAppE2ETests {
    ...
    func test_complete_sixteen_step_assembled_user_journey() async throws {
        ...
        // -------------------------------------------------------------------------
        let bundleURL = tempDir.appendingPathComponent("E2E_TestProject.amend")
        try appViewModel.saveProject(to: bundleURL)

        // -------------------------------------------------------------------------
        // Step 10: Verify .amend bundle on disk (project.json, audio/cues/cue_<UUID>.wav)
        // -------------------------------------------------------------------------
        ...
        // -------------------------------------------------------------------------
        // Step 16: Run CredentialLeakScanner on .amend bundle and output to verify zero credentials exist
        // -------------------------------------------------------------------------
