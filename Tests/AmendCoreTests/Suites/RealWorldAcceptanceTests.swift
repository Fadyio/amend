...
struct RealWorldAcceptanceTests {
    ...
    @Test(
        ...
        .enabled(if: ProcessInfo.processInfo.environment["AMEND_RUN_REAL_MEDIA_TESTS"] == "1")
    )
    @MainActor
    func test_real_media_acceptance_journey() async throws {
        guard let mediaPath = ProcessInfo.processInfo.environment["AMEND_REAL_MEDIA"], !mediaPath.isEmpty else {
            Issue.record("AMEND_REAL_MEDIA environment variable is not set. AMEND_RUN_REAL_MEDIA_TESTS=1 requires AMEND_REAL_MEDIA=\"/path/to/file.mov\".")
            return
        }

        let realMediaURL = URL(fileURLWithPath: mediaPath)
        guard FileManager.default.fileExists(atPath: realMediaURL.path) else {
            Issue.record("Real media file missing at '\(mediaPath)'. AMEND_RUN_REAL_MEDIA_TESTS=1 requires an existing media file.")
            return
        }
        ...
        // 7. Save / Reopen .amend restores media and Cues
        let bundleURL = FileManager.default.temporaryDirectory.appendingPathComponent("RealWorldAcceptanceTest.amend")
        ...
        // 8. Launch packaged dist/Amend.app with the saved bundle and capture genuine screenshot
        let projectRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let appURL = projectRoot.appendingPathComponent("dist/Amend.app/Contents/MacOS/Amend")
