...
struct VideoPlaybackRegressionTests {
    ...
    private func createTempDir() throws -> URL {
        let temp = FileManager.default.temporaryDirectory.appendingPathComponent("amend_video_reg_\(UUID().uuidString)")
    ...
    func test_saved_project_restores_playable_media() async throws {
        ...
        let bundleURL = tempDir.appendingPathComponent("TestProject.amend")
    ...
    @Test("Generate Demo Bundle in /tmp/amend_demo for interactive inspection")
    @MainActor
    func test_generate_demo_bundle() async throws {
        ...
        let bundleURL = sampleDir.appendingPathComponent("HackathonDemo.amend")
    ...
    func test_render_phase20_screenshots() async throws {
        ...
        let movieURL = URL(fileURLWithPath: "/tmp/amend_demo/HackathonDemo.mov")
        ...
        let sampleCues: [Cue] = [
            Cue(
                ...
                text: "Welcome to Amend. Today we are repairing the hackathon demo narration without re-recording any screen video.",
                originalText: "Welcome to Amend. Today we are repairing the hackathon demo narration without re-recording any screen video.",
                editState: .original,
                words: [
                    ...
                    WordTiming(word: "Amend.", timeRange: CMTimeRange(start: CMTime(seconds: 1.05, preferredTimescale: 600), duration: CMTime(seconds: 0.5, preferredTimescale: 600))),
        ...
        if let png = captureView(view4) {
            ...
            try png.write(to: screenshotsDir.appendingPathComponent("5_running_app_reference_monitor.png"))
        }

        if let png = captureView(view2) {
            try png.write(to: screenshotsDir.appendingPathComponent("macos26_liquid_glass.png"))
        }
        ...
        #expect(FileManager.default.fileExists(atPath: screenshotsDir.appendingPathComponent("5_running_app_reference_monitor.png").path))
        #expect(FileManager.default.fileExists(atPath: screenshotsDir.appendingPathComponent("macos26_liquid_glass.png").path))
    }
    ...
    func test_unsaved_changes_tracking_across_cue_operations() async throws {
        ...
        // 2. Save resets dirty
        let bundleURL = tempDir.appendingPathComponent("DirtyTest.amend")
        try appVM.saveProject(to: bundleURL)
